...
void table3D_setSize(struct table3D *targetTable, byte newSize) {
  ...
  for (byte i = 0; i < newSize; i++) {
    targetTable->values[i] = (byte *)malloc(newSize * sizeof(byte));
  }
...
int table2D_getValue(struct table2D *fromTable, int X) {
  ...
  */
  ...
  if (fromTable->valueSize == SIZE_BYTE)  {
    ...
    if (fromTable->values[xMax] > fromTable->values[xMin]) {
      yVal = fromTable->values[xMin] + yVal;
    }   else {
      yVal = fromTable->values[xMin] - yVal;
    }
  }  else  {
    ...
    if (fromTable->values[xMax] > fromTable->values16[xMin]) {
      yVal = fromTable->values16[xMin] + yVal;
    }    else {
      yVal = fromTable->values16[xMin] - yVal;
    }
  }
